...
class NavigationActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //vals from xml
        val editableTextView : TextView = findViewById(R.id.editText)
        val searchButton : Button = findViewById(R.id.nntuapp_searchButton)
        val prevFloorButton : ImageButton = findViewById(R.id.prevButton)
        var nextFloorButton : ImageButton = findViewById(R.id.nextButton)
        val floorText : TextView = findViewById(R.id.floorText)
        val theImage : ZoomageView = findViewById(R.id.theImage)
        val moreRooms : Button = findViewById(R.id.moreRoomsButton)
        val rotateButton : ImageButton = findViewById(R.id.rotateButton)

        val buildingText : TextView = findViewById(R.id.buildingText)
        val prevBuildingButton : ImageButton = findViewById(R.id.prevBuildingButton)
        val nextBuildingButton : ImageButton = findViewById(R.id.nextBuildingButton)
        ...
        fun showEmptyFloors() {
            try {
                ...
                val newImage : Drawable = getImage(this, name)
...
//        val input = if (isAdded) this.requireActivity().intent.getStringExtra("room") else null
        val input = this.intent.getStringExtra("room")
        if (input != null){
            ...
            try {
                ...
                Toast.makeText(this, getString(R.string.Аудитория_не_является_числом), Toast.LENGTH_SHORT).show()
            }
            showTheImage(input)
        }

        moreRooms.setOnClickListener {
            val goToIntent = Intent(this, MoreRoomsActivity::class.java)
            goToIntent.putExtra("nowBuilding", building)
            this.startActivity(goToIntent)
        }

        if (getDefaults("isUpdating", this) ?: 0 == 0){
        ...
        val rotate = AnimationUtils.loadAnimation(this, R.anim.rotate_center)
